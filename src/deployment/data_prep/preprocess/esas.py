...
def get_symptoms_data(esas_data_file, rename_legacy=False):
    ...
    col_map = {}
    if rename_legacy:
        # rename columns to match legacy model feature names (e.g. esas_anxiety, patient_ecog)
        col_map.update({c: f"esas_{c}" for c in SYMP_COLS})
