...
"""
# from make_clinical_dataset.epic.preprocess.esas import get_epic_symptom_data
# from make_clinical_dataset.shared.constants import SYMP_COLS


# def get_symptoms_data(esas_data_file):
#     df = get_epic_symptom_data(esas_data_file)
...
def process_symptoms_data(df):
    ...
    df = (
        df.groupby(["mrn", "survey_date"]).agg(
            {col: "mean" for col in cols}
        )  # handle conflicting data by taking the mean
    )
    df = df.reset_index()
...
def filter_symptoms_data(df):
    ...
    # clean data types
    for col in ["date_of_birth", "survey_date"]:
